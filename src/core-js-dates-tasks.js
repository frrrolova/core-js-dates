...
function getCountDaysInMonth(month, year) {
  ...
  const short = [4, 6, 9, 12];
  if (long.includes(month)) {
    return 31;
  }
  if (short.includes(month)) {
    return 30;
  }
...
function isDateInPeriod(date, period) {
  ...
  return (
    dateTime <= new Date(period.end).getTime() &&
    dateTime >= new Date(period.start).getTime()
  );
}
...
function formatDate(date) {
  ...
  return `${dateObj.getUTCMonth() + 1}/${dateObj.getUTCDate()}/${dateObj.getUTCFullYear()}, ${currentHours}:${minutes}:${seconds} ${dayPart}`;
}
...
function getWeekNumberByDate(date) {
  ...
  let fullWeeks = Math.floor(diffInDays / 7);
  const remainder = diffInDays % 7;
  const offset = yearStart.getDay() === 0 ? 6 : yearStart.getDay() - 1;
  if (remainder || (!remainder && offset)) {
    fullWeeks += 1;
  }
  if (remainder + offset >= 7) {
    fullWeeks += 1;
  }
  return fullWeeks;
}
...
function getNextFridayThe13th(date) {
  ...
  const month = day > 13 ? date.getMonth() + 1 : date.getMonth();
  let year = date.getFullYear();
  for (let m = month; m < 12; m += 1) {
    const checkedDate = new Date(year, m, 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const start = getNewDate(period.start);
  const end = getNewDate(period.end);
  const result = [];
  let counter = countWorkDays;
  while (start.getTime() <= end.getTime()) {
    if (counter) {
      result.push(getStringFromDate(start));
